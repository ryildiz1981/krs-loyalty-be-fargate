...
import {
    ...
    WEB_SOCKET_COMMUNICATION_SECRET, WEB_SOCKET_RECEIPT_CHANNEL
...
io.use((socket: any, next) => {
    ...
    webSocket.on(WEB_SOCKET_RECEIPT_CHANNEL, (printDto: ReceiptPrintRequestDto) => {
        if(process.env.USE_TEST_CARD_NUMBER && process.env.TEST_CARD_NUMBER) {
            printDto.cardNumber = process.env.TEST_CARD_NUMBER;
        }
        webSocket.emit(WEB_SOCKET_RECEIPT_CHANNEL, receiptService.createReceipt(printDto));
    })
